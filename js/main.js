// Utilizacion de funcion async y await para la recoleccion de la data contenida en el JSON que busco mediante fetch()

async function mostrarServicios() {
  const rta = await fetch("./servicios.json")
  const servicios = await rta.json()
  let listado = ""

    servicios.forEach(servicio => {
        listado += `<div class="col-md-4 text-center my-3">
        <img src="${servicio.imagen}" alt="${servicio.nombre}" height="72"/>
